...
/* ------------------------------------------- */
/*
	The following functions are from the program 'hello_ll.c' which is an example of using the fuse interface  
*/
struct dirbuf {
    char *p;
    size_t size;
...
// fuse v2	
    ...
    if (fuse_parse_cmdline(&args, &mountpoint, &multi, &foreground) != 0)
        return 1;
    ...
    se = fuse_lowlevel_new(&args, &fat16_ops, sizeof(fat16_ops), fs);
